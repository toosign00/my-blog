...
import { PostViewsLoader } from './PostViewsLoader';
...
export const PostGrid = ({ posts, className, ...props }: PostGridProps) => {
  ...
  const handleLoad = useCallback((map: Record<string, number>) => setViewsMap(map), []);

  return (
    <>
      <PostViewsLoader slugs={slugs} onLoad={handleLoad} />
      <div
      ...
      >
        {posts.map(({ _id, slug, title, coverImage, coverImageBlur, createdAt }, index) => {
          const views = viewsMap[`/posts/${slug}`];
          return (
            <Link
