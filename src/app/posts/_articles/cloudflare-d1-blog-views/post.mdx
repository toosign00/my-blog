...
새로고침할 때마다 카운트가 올라가면 곤란해서 익명 방문자 쿠키를 하나 뒀습니다. 쿠키가 없으면 `crypto.randomUUID()`로 새 값을 만들고, 서버에서는 `visitor:<uuid>` 형태의 키로 사용합니다.
...
새로 만든 UUID는 응답 쿠키에 저장합니다. JavaScript에서 읽을 필요가 없으므로 `httpOnly`로 설정했고, 현재 유지 기간은 180일입니다.
...
물론 쿠키만으로 방문자를 완전히 식별할 수는 없습니다. 쿠키를 지우거나 시크릿 창, 다른 브라우저나 기기를 사용하면 새로운 방문자로 집계됩니다.

기록하기 전에는 `pathname`도 검증합니다. 홈(`/`)과 실제로 존재하는 `/posts/{slug}`만 허용해 임의의 경로가 DB에 계속 생기지 않게 했습니다. 빈 문자열, 160자를 넘는 값, 쿼리 문자열이나 해시가 포함된 값도 거부합니다.
...
조회수는 글 본문보다 덜 중요한 정보입니다. 페이지를 먼저 정적으로 보여주고 조회수만 클라이언트에서 따로 가져오도록 바꿨습니다. 게시물 카드와 글 상세 화면은 같은 조회 훅을 사용하고, 숫자가 오기 전에는 `- views`를 보여줍니다.
...
오래된 조회수가 화면에 남지 않도록 API 응답과 fetch 모두 `no-store`로 맞췄습니다. TanStack Query도 `staleTime: 0`, `gcTime: 0`, `refetchOnMount: 'always'`로 설정했습니다.
